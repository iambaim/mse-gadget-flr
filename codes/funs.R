...
##--------------------------------------------------------------------
## specify the configuration of the assessment model
## submodel for fishing mortlaity (F)
...
#--------------------------------------------------------------------
## compute F for the projection years
...
#--------------------------------------------------------------------
## add uncertainty using the random walk (RW) model or correlation matrix
setMethod("genFLQuant", c("FLQuant"),
    function(object, cv = 0.2, method = "rw", niter = 250) {
  ## log-transform, to be expanded on later versions
	mu <- log(object)
	if(method == "ac") {
	...
	if(method == "rw") {
		...
		form <- x ~ f(years, 
		              ...
		              replicate = ages)
		data <- list(x = c(mu), 
		             years = rep(1:n.years, each = n.ages), 
		             ages  = rep(1:n.ages, n.years))
		result <- inla(form, 
		               data = data, 
		...
		## specify the covariance structure of the fitted RW
		RW.cov <- result$misc$ lincomb.derived.correlation.matrix
		
		## two options for the mean:
		##  1) use the mean estimate of the random walk (RW) process from INLA
